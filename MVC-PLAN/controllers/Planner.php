...
class Planner extends CIO_controller  {
    ...
    public function send_planner() 
	{
		...
		if(isset($_POST['form_example'])) {
			...
			$this->form_validation->set_rules('city', 'City', 'trim|required');	//varos
			$this->form_validation->set_rules('address', 'Address', 'trim|required'); //cim
			$this->form_validation->set_rules('postcode', 'PostCode', 'trim|required'); // iranyitoszam
			...
			if($valid == 1)
		    {
				...
				$msg = '
					...
					<b>Város: </b> '.$_POST['city'].'<br><br>
					<b>Cím: </b> '.$_POST['address'].'<br><br>
					<b>Irányítószám: </b> '.$_POST['postcode'].'<br><br>
